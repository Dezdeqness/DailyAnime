...
interface TopicListNamespace {
    data class State(
        val list: List<TopicListUiModel> = listOf(),
        val status: TopicListStatus = TopicListStatus.Initial,
    ...
    sealed class Event {
        ...
        data class OnPageLoaded(val list: List<TopicListUiModel>, val hasNextPage: Boolean) : Event()
        data class OnLoadMorePageLoaded(val list: List<TopicListUiModel>, val hasNextPage: Boolean) : Event()
